...
// map integer coordinates inside a specified cube map face
// into floating-point normalized coordinates (direction vector)
// get the 3D direction vector, which points from the center of cube to the specific pixel on a face
vec3 faceCoordsToXYZ(int i, int j, int faceID, int faceSize)
{
	// compute normalized coordinates, and the range is [0,2]
	const float A = 2.0f * float(i) / faceSize;
	const float B = 2.0f * float(j) / faceSize;

	// minus 1.0 makes the range to be [-1, 1]
	// A - 1.0 or 1.0 - A depends on face orientation
	if (faceID == 0) return vec3(-1.0f, A - 1.0f, B - 1.0f); // x = -1.0f means that the cube face is -X
	if (faceID == 1) return vec3(A - 1.0f, -1.0f, 1.0f - B); // y = -1.0f means that the cube face is -Y
...
Bitmap convertEquirectangularMapToVerticalCross(const Bitmap& b)
{
	...
	// the size of each cube face is 1/4 of the width of equirectangular map
	const int faceSize = b.w_ / 4;

	// compute the width and height of the vertical cross
	const int w = faceSize * 3;
	const int h = faceSize * 4;

	Bitmap result(w, h, b.comp_, b.fmt_);

	// the location (start position) of each cube map in the vertical cross format 
	...
	// iterating over the 6 cube map faces and each pixel inside each face
	for (int face = 0; face != 6; face++)
	{
		for (int i = 0; i != faceSize; i++)
		{
			...
			{
				// calculate the cartesian (XYZ) coordinates (direction vector) firstly
				const vec3 P = faceCoordsToXYZ(i, j, face, faceSize);

				// then convert the XYZ coordinates to the spherical coordinates
				...
				// based on the floating coordinates, we can get two integer values for each coordinate
				...
				// fractional part (is also the interpolation weights)
				const float s = Uf - U1;
				const float t = Vf - V1;
				// fetch 4-samples
				...
				// use the start position (kFaceOffsets) for setting pixel values
				// so that the result is the vertical cross format
...
// Vertical Cross needs to be converted to six separate 2D images (cube map texture)
    ...
    for (int j = 0; j != faceHeight; ++j) {
      for (int i = 0; i != faceWidth; ++i) {
        ...
        int y = 0;
		  ...
		  // get the x y coordinates in the vertical cross
		  // based on i j coordinates in each face
        switch (face) {
          // CUBE_MAP_POSITIVE_X
        case 0:
          x = 2 * faceWidth + i;

...
struct Bitmap
{
	...
	int w_ = 0;
	int h_ = 0;
	int d_ = 1; // array layers (6 for cube map)
	int comp_ = 3; // the number of components per pixel
	eBitmapFormat fmt_ = eBitmapFormat_UnsignedByte; // unsigned byte or float (the type of a single component)
	eBitmapType type_ = eBitmapType_2D; // 2D texture or cube map
	std::vector<uint8_t> data_; // store actual pixel data (using vector for simplicity)

...
public:
  ...
  // pred is a filter, filtering the draw commands
  ...
  DrawIndexedIndirectCommand* getDrawIndexedIndirectCommandPtr() const
  {
    LVK_ASSERT(ctx_->getMappedPtr(bufferIndirect_));
	 ...
	 // plus sizeof(uint32_t) is for skipping the block size belonging to the draw commands counts
	 // cast the memory pointer's type from uint_8* to DrawIndexedIndirectCommand*
...
public:
  ...
  , textureFiles_(meshData.textureFiles)
  {
    ...
    // prepare indirect commands buffer
	 // loop for every mesh node to populate the draw commands
    for (auto& i : scene.meshForNode) {
	 ...
	 // upload the data to the indirect buffer
    indirectBuffer_.uploadIndirectBuffer();

	 // create and upload the data to draw data buffer
  ...
  // we can use different indirect command buffers for the same VKMesh object
  // to render the different parts of the mesh (like the opaque part or transparent part)
  // fixed push constants in this version of draw function
  void draw(
  ...
  // this draw function takes custom push constants and depth state 
  void draw(
      ...
      const VKIndirectBuffer11* indirectBuffer = nullptr) const
  {
    ...
    if (!indirectBuffer)
	 ...
	 // the draw commands counter is in the bufferIndirect_, and the offset is 0 (very beginning of the buffer)
    buf.cmdDrawIndexedIndirectCount(
        indirectBuffer->bufferIndirect_, sizeof(uint32_t), indirectBuffer->bufferIndirect_, 0, numMeshes_,
        sizeof(DrawIndexedIndirectCommand));
  }

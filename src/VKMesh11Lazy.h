...
// similar to loadTexture function, but with key difference (not create the Vulkan texture)
// it returns the texture desc data (necessary for creating texture), but doesn't create the texture now
    ...
    std::vector<LoadedTextureData>& loadedTextureData, std::mutex& loadingMutex)
{
  ...
  // lock guard and mutex are used together
  // we don't need to unlock manually
  // only one thread can access the code in the scope at same time
...
// VKMesh11 is VKMesh11Lazy's base class
...
public:
  VKMesh11Lazy(
	  ...
	  // call the constructor of base class before running own constructor
  : VKMesh11(ctx, meshData, scene, indirectBufferStorage, false)
  {
    materialsGPU_.resize(materialsCPU_.size());

    // construct Taskflow
	 // step is 1, which means i will be 1, 2, 3, 4...
    ...
    // start loading
	 // the taskflow runs asynchronously (not on main thread)
    executor_.run(taskflow_);
  }

  // the Vulkan texture creation can only be done on main thread
  // so this function (creating Vulkan texture) is on the main thread
  bool processLoadedTextures()
  {
    ...
    LoadedTextureData tex;
	 ...
	 // the {} controls the scope of lock guard
	 // process 1 texture at a time (every frame)
	 // mutex here blocks other background threads to access loadedTextureData_ and textureCache_, when the main thread access them
	 // the texture creation is outside the mutex scope, because it's the slow part and no relation to background thread's work (loading texture data)
    {
    ...
    lvk::Holder<lvk::TextureHandle> texture = ctx->createTexture(tex.desc);
	 ...
	 // destroy the loaded ktx texture data as it's no longer needed
    ktxTexture_Destroy(ktxTexture(tex.ktxTex));

    {
      std::lock_guard lock(loadingMutex_);

		// texture cache stores the created Vulkan textures (not loaded ktx texture data)
      textureCache_[tex.index] = std::move(texture);

		// using this local lambda to keep the code clean
      ...
      // go through the texture cache and update materials
		// map the texture index from CPU to GPU (descriptor array)
	 ...
	 // upload the materialsGPU data to the GPU buffer

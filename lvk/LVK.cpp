...
#error Unsupported OS
  ...
  // enumerate Vulkan physical devices and select the most perferred one
  // priortize choosing a discrete GPU first, and if none is available than choose intergate one
  // if no dicrete one, opt integrated one
  if (!numDevices) {
  ...
  // once we selected the device, then call initContext to create all Vulkan internal data structures

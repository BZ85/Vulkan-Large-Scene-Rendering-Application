...
// no buffers, since all buffers are accessed by GPU device address in shader, not bound to descriptor sets
enum Bindings {
  kBinding_Textures = 0,
  kBinding_Samplers = 1,
...
// convert lvk loadOp bit to Vk attachment loadOp bit
...
// convert lvk storeOp bit to Vk attachment storeOp bit
VkAttachmentStoreOp storeOpToVkAttachmentStoreOp(lvk::StoreOp a) {
  ...
  case lvk::StoreOp_MsaaResolve:
    // for MSAA resolve, we have to store data into a special "resolve" attachment
	 // the MSAA color attachment's content is not needed after the pass, so using DontCare
	 // the resolve data will be stored into the resolve attachment mentioned above
    return VK_ATTACHMENT_STORE_OP_DONT_CARE;
...
// transition the texture(to be the render target)'s layout to color attachment optimal 
...
// read data from GPU back to CPU
void lvk::VulkanBuffer::getBufferSubData(const VulkanContext& ctx, size_t offset, size_t size, void* data) {
  ...
  // necessary for non-coherent memory
...
// transition the layout of the image (texture), which also means inserting a image barrier
void lvk::VulkanImage::transitionLayout(VkCommandBuffer commandBuffer,
  ...
  // VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL includes both color attachment and depth attachment
  // we can still identify whether the current image layout is color or depth
...
// necessary for rendering into cube map
...
lvk::VulkanPipelineBuilder& lvk::VulkanPipelineBuilder::rasterizationSamples(VkSampleCountFlagBits samples, float minSampleShading) {
  ...
  // minSampleShading is a ratio of the shading samples per pixel, but not a number
...
// transition the layout of the texture images (inserting the barriers)
void lvk::CommandBuffer::transitionToShaderReadOnly(TextureHandle handle) const {
  ...
  if (img.vkSamples_ == VK_SAMPLE_COUNT_1_BIT) {
    ...
    // set the result of the previous render pass
	 // shader read only layout is for sampled image (sampling only)
	 // general layout is for storage image, which can be written and read in any shaders including compute shaders 
	 // the subresource range here covers the entire mip levels, base level is 0 and level count is all the remaining, (not support for specific mip level)
...
void lvk::CommandBuffer::cmdBindComputePipeline(lvk::ComputePipelineHandle handle) {
  ...
  // lazily create the compute pipeline here
  VkPipeline pipeline = ctx_->getVkPipeline(handle);
  ...
  // check whether there is new pipeline to be bound
  if (lastPipelineBound_ != pipeline) {
    lastPipelineBound_ = pipeline;
...
void lvk::CommandBuffer::cmdDispatchThreadGroups(const Dimensions& threadgroupCount, const Dependencies& deps) {
  ...
  // transit the layout (insert barriers) for textures used in compute shader
...
// transition the layout for texture used in compute shader
void lvk::CommandBuffer::useComputeTexture(TextureHandle handle, VkPipelineStageFlags2 dstStage) {
  ...
  // transit to general layout for storage image, and to shader read only optimal for sampled image
...
void lvk::CommandBuffer::cmdBeginRendering(const lvk::RenderPass& renderPass, const lvk::Framebuffer& fb, const Dependencies& deps) {
  ...
  // add barriers (transition the layouts) for all the texture images (going to be read in shaders)
  for (uint32_t i = 0; i != Dependencies::LVK_MAX_SUBMIT_DEPENDENCIES && deps.textures[i]; i++) {
  ...
  // make sure all the texture images (going to be the render targets)'s layout to be transit to color attachment optimal
  ...
  // transition the image layout of the depth texture to depth stencil attachment optimal
  ...
  // loop for every LVK color attachments, to convert them into VkRenderingAttachmentInfo
  for (uint32_t i = 0; i != numFbColorAttachments; i++) {
    ...
    colorAttachments[i] = {
        ...
        .imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
		  // resolveMode defines how the multisampled data is resolved
        ...
        .loadOp = loadOpToVkAttachmentLoadOp(descColor.loadOp),
		  // convert the LVK storeOp to Vulkan attachment one
    ...
    if (descColor.storeOp == StoreOp_MsaaResolve) {
      ...
      lvk::VulkanImage& colorResolveTexture = *ctx_->texturesPool_.get(attachment.resolveTexture);
		// take the VkImageView of the resolve texture and assign to color attachments (VkRenderingAttachmentInfo)
  ...
  // convert the LVK depth attachment into VkRenderingAttachmentInfo
  // similar to the color attachments
  ...
  // it serves the same function as the traditional render pass
  const VkRenderingInfo renderingInfo = {
      ...
      .colorAttachmentCount = numFbColorAttachments,
		// assign the color attachments and depth attachments created above to the renderingInfo
  ...
  // default depth comparison function is op_always
  // but we will set it using cmdBindDepthState
...
void lvk::CommandBuffer::cmdBindRenderPipeline(lvk::RenderPipelineHandle handle) {
  ...
  // if the pipeline has already been created and cached, then return it
  // otherwise we move forward to build a new Vulkan pipeline
  VkPipeline pipeline = ctx_->getVkPipeline(handle);
...
// dynamically set the depth comparison function
...
void lvk::CommandBuffer::cmdUpdateBuffer(BufferHandle buffer, size_t bufferOffset, size_t size, const void* data) {
  ...
  LVK_ASSERT(buffer.valid());
  LVK_ASSERT(data);
  LVK_ASSERT(size && size <= 65536); // the buffer size should smaller than 64kb (65536 bytes)
  LVK_ASSERT(size % 4 == 0);
  LVK_ASSERT(bufferOffset % 4 == 0);
...
void lvk::CommandBuffer::cmdClearColorImage(TextureHandle tex, const ClearColorValue& value, const TextureLayers& layers) {
  ...
  // when clearing the color image, the pipeline stage is at the transfer bit
  // the access mask is TRANSFER_WRITE_BIT
  // the image layout should be TRANSFER_DST_OPTIMAL
  lvk::imageMemoryBarrier2(
      wrapper_->cmdBuf_,
      img->vkImage_,
      StageAccess{.stage = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT},
...
lvk::Holder<lvk::TextureHandle> lvk::VulkanContext::createTexture(const TextureDesc& requestedDesc,
  ...
  if (desc.usage & lvk::TextureUsageBits_Attachment) { // is a render target (attachment) or not
    usageFlags |= lvk::isDepthOrStencilFormat(desc.format) ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT
	 ...
	 // set the transient attachments like MSAA texture or depth texture (if not used after the pass) to not require the physical storage
...
lvk::Holder<lvk::TextureHandle> lvk::VulkanContext::createTextureView(lvk::TextureHandle texture,
  ...
  // memset is used to set the value for a block of memory
  // here it is used to clear (erase) the objects for imageView
  ...
  // Only sampled images support swizzling, and storage images don't support swizzling
  if (image.vkUsageFlags_ & VK_IMAGE_USAGE_STORAGE_BIT) {
    if (!desc.swizzle.identity()) {
      // use identity swizzle for storage images
  ...
  // transfer the ownership from image to the pool, and no copying of the image resource
  TextureHandle handle = texturesPool_.create(std::move(image));
...
#undef UPDATE_PUSH_CONSTANT_SIZE
  ...
  lvk::VulkanPipelineBuilder()
      ...
      .primitiveTopology(topologyToVkPrimitiveTopology(desc.topology))
	   // set the MSAA sample counts and min number (ratio) of shading sample counts
...
// lazily create the Vulkan compute pipeline
VkPipeline lvk::VulkanContext::getVkPipeline(ComputePipelineHandle handle) {
  ...
  // if the pipeline has not been created and cached for this compute pipeline state
  // then create this compute pipeline
...
// copy data from the mapped GPU buffer back to CPU
// only for the buffer is host visible
  ...
  if (strstr(source, "#version ") == nullptr) {
    ...
    if (vkStage == VK_SHADER_STAGE_FRAGMENT_BIT) {
      ...
      // https://github.com/KhronosGroup/Vulkan-Samples/blob/main/shaders/descriptor_indexing/nonuniform-quads.frag#L33-L39
		// nonuniformEXT() should be used when the indexing of the descriptor array is not dynamically uniform (same) across all the invocations
		sourcePatched += R"(
      #version 460
      #extension GL_EXT_buffer_reference_uvec2 : require
      #extension GL_EXT_debug_printf : enable
...
lvk::Result lvk::VulkanContext::initContext(const HWDeviceDesc& desc) {
  ...
  VkPhysicalDeviceVulkan11Features deviceFeatures11 = {
      ...
      .multiview = vkFeatures11_.multiview, // enable the multiview feature if supported, required for rendering shadow cubemap faces in one pass
...
#undef CHECK_FEATURE_1_3
  ...
  // default sampler
  // default sampler is created when the Vulkan context is initialized, and its index is 0
  ...
  createSampler(
      {
          ...
          .minFilter = VK_FILTER_LINEAR,
			 // set the trilinear filtering (between mipmap levels)
          ...
          .maxAnisotropy = 0.0f,
          .compareEnable = VK_FALSE, // not enable the comparison operation since it's not shadow sampler
          .compareOp = VK_COMPARE_OP_ALWAYS,
          .minLod = 0.0f,
...
void lvk::VulkanContext::checkAndUpdateDescriptorSets() {
  ...
  // create one VkWriteDescriptorSet structure for each of our four bindings
  // YUVImages is a standalone binding, separated from sampled images and storage images
  ...
  if (!infoYUVImages.empty()) {
    write[numWrites++] = VkWriteDescriptorSet{
        ...
        .descriptorCount = (uint32_t)infoYUVImages.size(),
		  // YUV image's descriptor type is combined image sampler
